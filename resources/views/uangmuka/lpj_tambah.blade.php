...
<script>
let rowIndex = 1;
function tambahBaris(){const b=document.querySelector('#detailTable tbody');const tr=document.createElement('tr');tr.style.borderBottom='1px solid #f1f3f4';tr.innerHTML=`<td class="text-center row-number fw-semibold text-secondary">${b.children.length+1}</td><td><input type="text" name="items[${rowIndex}][keterangan]" class="form-control form-control-custom" placeholder="Rincian realisasi..." required></td><td><input type="text" name="items[${rowIndex}][kode_budget]" class="form-control-custom" placeholder="Kode Budget" required></td><td><div class="input-group input-group-sm"><span class="input-group-text bg-transparent border-end-0 px-2" style="font-size:12px;color:#6b7280;">Rp</span><input type="text" name="items[${rowIndex}][jumlah]" class="form-control form-control-custom input-jumlah" placeholder="0" required inputmode="numeric" oninput="formatNominal(this); hitungTotal()"></div></td><td class="text-center"><button type="button" class="btn btn-sm btn-outline-danger border-0 rounded-circle" onclick="hapusBaris(this)"><i class="fa-solid fa-trash"></i></button></td>`;b.appendChild(tr);rowIndex++;}
